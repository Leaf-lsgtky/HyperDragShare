...
本文记录 HyperDragShare `1.7.20`（`versionCode 44`）的当前完整实现、关键兼容性选择和已验证
...
两种窗口来源都使用相同的背景透明度路径。“图标不透明度”同时作用于三种样式的目标图标
...
去重后加载图标和名称，按菜单方向选择横向或纵向滚动容器。“复制”是文字和图片菜单共同的
内置首项；图片菜单随后是“保存到本地”，文字菜单随后是“文本分词”。文字复制使用普通文本
剪贴板，图片复制会等待 JPEG 暂存完成后以 `ClipData.newUri()` 写入可读取的图片 URI。文字和
图片复制开关分别控制其对应菜单中的复制项，所有内置动作也都遵守菜单可见性设置；保存/分词
不会出现在另一种内容类型的菜单中。分词页运行在模块进程，使用本地 `cppjieba` 词典将文字拆成可选词块，
...
- 可见性页先把文字和图片 MIME 能处理的 Activity 合并，再按包名分组。列表结构按 KernelSU
  ...
  点击部分选择会全选，点击全选会全部隐藏。子项 Checkbox 仍可独立控制。内置“复制”“保存到本地”和“文本分词”
  位于“内置功能”组。
- 可见性页顶部操作菜单提供“全选/全不选/全展开/全折叠”；排序页使用稳定的 `ComponentName.flattenToString()` 作为键，采用 XiaomiHelper 同款的
  ...
  新应用会在保存顺序之后自动追加。内置“复制”“保存到本地”和“文本分词”不进入可排序应用列表，运行时固定
  置于各自菜单的应用项之前（可见性仍可单独关闭）；在可见性页关闭的目标不会进入排序列表。
  “复制”和“保存到本地”显示为与其他应用图标同尺寸容器内的强调色圆角瓷片。可见性页的瓷片缩为
...
| `text_copy_enabled` | 是否在文字菜单显示复制 | `true` |
| `image_copy_enabled` | 是否在图片菜单显示复制 | `true` |
...
- root ACTION_UP：先回放暂存的宿主 `cancelTask` / `257` 原方法，使传送门任务可再次触发；
  随后更新最后落点、停止边缘滚动，并在预览/菜单/光效窗口仍附着时请求启动选中目标，最后移除这些窗口。
  无障碍服务在 HyperOS 上失去最后一个可见 `TYPE_ACCESSIBILITY_OVERLAY` 后可能被静默拒绝后台启动，
  因此外部分享和文本分词都必须保持这个启动顺序。
...
- 命中“复制”：文字立即写入剪贴板；图片等待同一暂存 URI 后写入图片 `ClipData`，成功或失败均显示 Toast。
...
当前 62 个单元测试覆盖：当前版本注入握手与限定服务启动命令、底部触发边界、左右滚动方向、
...
候选优先级、截图区域的扩边/缩放/夹取，以及背景锁开关的 Provider Bundle 同步、root 服务返回
