...
本文记录 DragShare `1.7.6`（`versionCode 30`）的当前完整实现、关键兼容性选择和已验证
...
- Android：minSdk 28，targetSdk 34，compileSdk 37
- LSPosed API：82
...
`content://com.example.dragshare.share/settings`：传送门进程收到无障碍模式时会取消会话并停止
...
```text
PortalHooks
    -> ContentResolver.call(content://com.example.dragshare.share, "get_settings")
...
```text
content://com.example.dragshare.share/shared/<UUID>.jpg
```
